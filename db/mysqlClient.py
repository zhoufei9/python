...
from setting import CONFIG_CONST
...
class OpenDB(object):
    def __init__(self):
        ...
        # 初始化
        self.conn = connect(
            host=CONFIG_CONST['mysql_config']['host'],
            port=CONFIG_CONST['mysql_config']['port'],
            user=CONFIG_CONST['mysql_config']['user'],
            passwd=CONFIG_CONST['mysql_config']['passwd'],
            db=CONFIG_CONST['mysql_config']['db'],
            charset=CONFIG_CONST['mysql_config']['charset']
        )
        # 获取游标
...
class DB(object):
    def __init__(self, database):
        ...
        self.order_by = ''
        self.group_by = ''
        if CONFIG_CONST['mysql_config']['pre']:
            self.database = CONFIG_CONST['mysql_config']['pre'] + database
        else:
    ...
    # 更新内容
    def increment(self, field=None):
        # 条件判断
        if not field:
            raise Exception('字段不能为空')

        # 合并dict并转化为元组
        val_list = tuple(list(self.where_vals))
        
        try:
            with OpenDB() as cs:
                sql = "UPDATE {} SET " + field +" = " + field + " + 1 WHERE {}"
                sql = sql.format(self.database, self.where_keys)
                row = cs.execute(sql, val_list)
                return row
        except Exception as e:
            return e
