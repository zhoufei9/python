...
DB('user').insert({'username': '张三', 'password': '123456'})
2.批量数据插入，传入list列表数据组
DB('user').insert([{'username': '张三', 'password': '123456'}, {'username': '李四', 'password': '123456'}])
...
DB('user').where('id', 1).where('username', '张三').update({'username': '王五'})
删除数据
delete()
和where()一起调用(查询方法里会介绍)，如果没有使用where()方法链式调用时会删除数据表的所以数据
返回值：bool值
...
DB('user').where('username', '小明').where('password', '123456').select()
2.单个where条件使用

DB('user').where({'username': '小明', 'password': '123456'}).select()
3.单个where和list同时使用多条件，例如：{'字段': ['比较符号'， '查询值', 'and|or']}，多个list时最后一个list的且或不计入比较

DB('user').where({'username': ['!=', '小明', 'or'], 'password': ['=', '123456']}).select()
field(params)
参数	必填	说明	举例
params	必填	查询字段，可是字符串或是字典	field('username')
查询字段，params为需要查询的字段，可以是字符串或是字典

例1：DB('user').field('username').find()
例2：DB('user').field('username,password').find()
例3：DB('user').field(['username', 'password']).find()
page(page=1, rows=10)
参数	必填	说明
page	非必填	分页页数
rows	非必填	分页行数
